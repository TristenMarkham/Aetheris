...
// 🔧 FIXED: Safe number formatting helper
...
// 🧠 NEW: AI-POWERED PROFIT CALCULATION FUNCTIONS
const calculateGrossProfit = (companyData) => {
  ...
  // 🧠 INTELLIGENT: Calculate revenue and labor costs for each client
  safeClients.forEach(client => {
    const clientRevenue = safeFormatNumber(client.monthly_revenue, 0);
    totalRevenue += clientRevenue;
    
    // 🧠 SMART: Get contract details if available
    ...
    // 🧠 DYNAMIC: Find guards assigned to this client (distribute proportionally for now)
    const activeGuards = safeEmployees.filter(e => e.status === 'Active');
    let clientLaborCost = 0;
    
    if (activeGuards.length > 0) {
      // 🧠 THINKING: Distribute hours among active guards
...
// 🧠 NEW: Advanced financial analysis with AI insights
const getFinancialAnalysis = (companyData) => {
  const profitData = calculateGrossProfit(companyData);
  const safeEmployees = companyData.employees || [];
  const activeGuards = safeEmployees.filter(e => e.status === 'Active');
  
  // 🧠 INTELLIGENT: Calculate business insights
  ...
  // 🧠 SMART: Estimate additional costs
...
// 🧠 AI-POWERED: Business recommendations based on profit analysis
...
// 🧠 AI-POWERED: Smart client update intent parsing
const parseClientUpdateIntent = async (userMessage, availableClients) => {
...
// 🧠 AI-POWERED: Intelligent deletion intent analysis
const parseModuleDeletionIntent = (userMessage, availableModules) => {
  console.log(`🧠 Analyzing deletion intent: "${userMessage}"`);
  
  const messageLower = userMessage.toLowerCase();
  
  // 🧠 PATTERNS: What indicates mass deletion vs. single item complaint?
  ...
  const singleItemComplaintPatterns = [
    /that mod.*didn't have/,
    /it didn't have/,
    /this module.*doesn't/,
    /deleted that.*it/
  ];
  ...
  const isSingleItemComplaint = singleItemComplaintPatterns.some(pattern => pattern.test(messageLower));
  
  return {
    shouldDeleteAll: isMassDeleteRequest,
    isSingleComplaint: isSingleItemComplaint,
    reasoning: isMassDeleteRequest ? 'User explicitly requested mass deletion' :
               isSingleItemComplaint ? 'User is complaining about one item, not requesting mass deletion' :
               'Unclear deletion intent - ask for clarification',
    availableModules: availableModules.map(m => m.name),
    recommendedAction: isMassDeleteRequest ? 'delete_all_modules' :
                      isSingleItemComplaint ? 'ask_for_clarification' :
                      'request_specific_deletion_target'
  };
};

// SMART DATA PARSING FUNCTIONS
const parseEmployeeInput = async (inputText) => {
...
const handleUserCorrection = async (originalData, correctionText) => {
...
const parseContractDetails = async (contractDescription, clientName, hourlyRate) => {
...
Respond ONLY with JSON:
{
  ...
  "specialRequirements": "string"
    ...
    const calculationBreakdown = {
      ...
      revenue: `${parsed.totalWeeklyHours} hours × $${hourlyRate}/hr = $${safeFormatCurrency(weeklyRevenue)}/week`,
      monthly: `$${safeFormatCurrency(weeklyRevenue)}/week × 4.33 weeks = $${safeFormatCurrency(monthlyRevenue)}/month`
    };
...
// 🔧 VARIABLE-DRIVEN: Confirmation messages use actual data instead of hard-coded names
...
⚠️ **Manual confirmation required** - Will not auto-add`;
    ...
    default:
      return `⚠️ Confirmation needed for ${type}. Reply "Yes" to confirm or "No" to cancel.`;
  }
};
...
const applyUserCorrection = async (correctionText, originalConfirmationId = null) => {
  try {
    ...
    if (targetConfirmation.type === 'add_employee') {
      const correctionResult = await handleUserCorrection(targetConfirmation.data, correctionText);
...
// 🔧 VARIABLE-DRIVEN: Dynamic default data creation instead of hard-coded
const createDefaultCompanyData = (companyId) => {
  ...
  // 🔧 VARIABLE-DRIVEN: Start with empty data - no hard-coded employees/clients
  return {
    companyInfo,
...
const loadCompanyData = async (companyId) => {
  try {
    ...
    // 🔧 VARIABLE-DRIVEN: Use dynamic default data instead of hard-coded
...
const deleteSingleModule = async (companyId, moduleId) => {
  try {
    ...
    console.log(`🗑️ Found module to delete: ${moduleToDelete.name}`);
    ...
    companyData.platformModules = companyData.platformModules.filter(m => m.id !== moduleId);
    ...
    console.log(`✅ Module "${moduleToDelete.name}" deleted successfully (backup: ${backup.backupFile})`);
...
const addClient = async (companyId, clientData) => {
  try {
    ...
    if (clientData.contractDescription && clientData.hourlyRate) {
      calculatedData = await parseContractDetails(
...
// 🧠 FIXED: AI-powered client update function with intelligent billing rate handling
const updateClient = async (companyId, clientId, updates) => {
  try {
    ...
    // 🧠 INTELLIGENT: Find client by ID (number) or name (string)
    let clientIndex = -1;
    ...
    // 🧠 SMART: Handle billing rate updates with automatic revenue recalculation
    if (updates.billingRate || updates.hourlyRate) {
      const newRate = updates.billingRate || updates.hourlyRate;
      console.log(`💰 AI updating billing rate to: $${newRate}/hr`);
      ...
      if (client.contract_details) {
        ...
        console.log(`📊 AI recalculated: ${weeklyHours}hrs/week × $${newRate}/hr = $${newMonthlyRevenue}/month`);
      } else {
        // Create new contract details with AI default estimation
        ...
        console.log(`📊 AI created contract: ${defaultWeeklyHours}hrs/week × $${newRate}/hr = $${newMonthlyRevenue}/month`);
      }
    }
    ...
    console.log(`✅ AI updated client ${client.name} successfully`);
...
// 🔧 FIXED: Enhanced deleteClient function for multiple matches
const deleteClient = async (companyId, clientIdentifier) => {
  try {
    ...
    // Search by name (partial, case insensitive) - THIS FINDS MULTIPLE "REAL RAIL"
    ...
    console.log(`✅ ACTUALLY DELETED ${clientsToDelete.length} clients: ${deletedNames}`);
...
// 🔧 NEW: Employee deletion function
const deleteEmployee = async (companyId, employeeIdentifier) => {
  try {
    ...
    console.log(`✅ ACTUALLY DELETED employee: ${employeeToDelete.name} (${originalCount} → ${newCount})`);
...
// 🧠 AI-POWERED: Smart business data analysis that thinks contextually
const getBusinessData = async (query) => {
  ...
  // 🧠 INTELLIGENT: Role-based categorization
  ...
  // 🧠 ADVANCED: Financial intelligence with profit analysis
  const financialAnalysis = getFinancialAnalysis(companyData);

  // 🧠 CONTEXTUAL: Business intelligence summary
  ...
  // 🧠 DYNAMIC: Dashboard metrics with intelligence
  const dashboardMetrics = {
    ...
    // 🧠 INTELLIGENT: Profit information
    ...
    // 🧠 SMART: Display values that adapt to context
    dashboardDisplays: {
  ...
  // 🧠 ADAPTIVE: Response data based on what user is asking for
  const responseData = {
    success: true,
    ...
    summary: {
      ...
      // 🧠 ADAPTIVE: Context-aware details
      activeGuards: realTimeMetrics.activeGuards,
      totalClients: realTimeMetrics.totalClients,
      monthlyRevenue: realTimeMetrics.monthlyRevenue,
      grossProfit: financialAnalysis.grossProfit,
      ...
      // 🧠 SMART: Detailed breakdowns when needed
  ...
  // 🧠 RETURN: Rich data for AI to think with
  return responseData;
};
...
const handleConfirmationResponse = async (message) => {
  ...
  if (!mostRecentConfirmation) {
    console.log(`⚠️ No pending confirmations found`);
    return {
  ...
  if (result.success && result.executed) {
    ...
    return {
      message: `⚠️ **Error:** ${result.error || 'Could not process confirmation'}`
    };
  }
};

// 🧠 VARIABLE-DRIVEN: AI-powered system prompt builder that thinks dynamically
const buildDynamicSystemPrompt = (companyData, realTimeMetrics, safeClients, safeEmployees, drivers) => {
  ...
  const businessContext = {
    ...
    avgHourlyRate: safeEmployees.length > 0 ? 
      Math.round(safeEmployees.reduce((sum, e) => sum + safeFormatNumber(e.hourlyRate, 0), 0) / safeEmployees.length) : 0
  };
...
Revenue Status: ${businessContext.hasRevenue ? `${safeFormatCurrency(realTimeMetrics.monthlyRevenue)}/month` : 'No revenue yet'}
...
- Modules: ${realTimeMetrics.totalModules}`;
  ...
  return `You are Aetheris, ${currentSession.userName}'s AI assistant for ${companyData.companyInfo.name}.
...
- NEVER call delete_all_modules unless user explicitly says "delete all modules"
- Complaints about one module ≠ delete everything
- Always ask for clarification if deletion intent is unclear
- Use specific delete functions: delete_client(), delete_employee(), deleteSingleModule()
...
app.post('/api/aetheris/chat', async (req, res) => {
  try {
    ...
    // 🔧 VARIABLE-DRIVEN: Load fresh company data for each chat
    const companyData = await loadCompanyData(currentSession.companyId);
    const userPersonalData = await getUserPersonalInfo(currentSession.companyId, currentSession.userId);

    // 🔧 VARIABLE-DRIVEN: Safe metric calculations using current data
    const safeClients = companyData.clients || [];
    const safeEmployees = companyData.employees || [];
    const safeSites = companyData.sites || [];
    const safeModules = companyData.platformModules || [];
    ...
    // 🔧 VARIABLE-DRIVEN: Role-based filtering using current employee data
    const staticGuards = safeEmployees.filter(e => 
      e.role === 'Security Guard' && e.status === 'Active'
    );
    ...
    // 🔧 VARIABLE-DRIVEN: Build system prompt from current database state
    const systemPrompt = buildDynamicSystemPrompt(companyData, realTimeMetrics, safeClients, safeEmployees, drivers);
    ...
    // 🧠 INTELLIGENT: Function requirement detection that thinks contextually
    const requiresFunction = (message) => {
      const messageLower = message.toLowerCase();
      
      // 🧠 SMART: Context-aware keyword detection
      ...
      // 🧠 CONTEXTUAL: Look for business intent, not just keywords
      const hasBusinessIntent = businessQueryPatterns.some(pattern => pattern.test(messageLower));
      
      // 🧠 SMART: Exclude obvious non-business queries
    ...
    if (completion.choices[0].message.tool_calls) {
      ...
      switch (functionName) {
        case 'get_weather':
          ...
          console.log(`🌤️ Getting weather for: ${weatherLocation} (original: ${functionArgs.location})`);
        ...
        case 'update_client':
          ...
          // 🧠 INTELLIGENT: Parse the user's intent from their message
          const updateIntent = await parseClientUpdateIntent(message, safeClients);
        ...
        case 'delete_all_modules':
          ...
          // 🧠 INTELLIGENT: Analyze user intent instead of blindly executing
          const deletionIntent = parseModuleDeletionIntent(message, safeModules);
          console.log(`🧠 Deletion analysis:`, deletionIntent);
          
          if (deletionIntent.shouldDeleteAll) {
            console.log(`✅ Confirmed mass deletion request`);
            functionResult = await deleteAllModules(currentSession.companyId);
            if (functionResult.success) dataChanges.push({ type: 'all_modules_deleted', count: functionResult.deletedCount, backupFile: functionResult.backupFile });
          } else {
            console.log(`🚨 PREVENTED accidental mass deletion`);
            functionResult = { 
              success: false, 
              error: `I understand you're ${deletionIntent.isSingleComplaint ? 'having issues with a module' : 'asking about deletion'}, but I prevented mass deletion to be safe.`,
              suggestion: deletionIntent.isSingleComplaint 
                ? 'Which specific module would you like me to help you with or remove?'
                : 'Please specify exactly what you want to delete, or say "delete all modules" if you really want to remove everything.',
      ...
      // 🧠 VARIABLE-DRIVEN: AI-powered follow-up that thinks contextually
...
// 🔧 FIXED: Module deletion endpoint with better logging
...
app.get('/api/business-metrics', async (req, res) => {
  try {
    ...
    // 🔧 VARIABLE-DRIVEN: Metrics calculation using actual current data
    const safeClients = companyData.clients || [];
    const safeEmployees = companyData.employees || [];
    const safeSites = companyData.sites || [];
    ...
    // 🧠 NEW: Add AI-powered profit calculations to metrics
    const financialAnalysis = getFinancialAnalysis(companyData);
...
// 🧠 NEW: AI-powered client billing rate update endpoint
...
// 🧠 NEW: AI-powered profit analysis endpoint
...
app.get('/api/health', (req, res) => {
  res.json({ 
    ...
    aiExamples: [
      ...
      '🧠 "that mod didn\'t work" → AI analyzes complaint vs deletion request → prevents mass deletion',
...
const startServer = async () => {
  ...
  console.log('🧠 AI-POWERED BUSINESS SYSTEM - READY!');
  ...
  console.log('  🧠 "that mod didn\'t work" → AI analyzes complaint vs deletion request → prevents mass deletion');
